...
// 替换url
function replaceUrl(fileContent, regpre, prefix, version) {
    ...
    var reg = new RegExp("(href=(\"|\')" + regpre + "(.+).css(\"|\'))|(src=(\"|\')" + regpre + "(.+)(\"|\'))", 'g');
    var urls = fileContent.match(reg);
    for (var i = 0; i < urls.length; i++) {
        var oldUrl = urls[i];
        var key = urls[i].replace(regpre, '/');
        // 提取引号里面的部分
        key = key.replace(/([^\?]*)(\"|\')+([^\?]*)(\"|\')+/g, '$3');
        if(version[key]) {
...
module.exports.act = function(buffer, encoding, callback) {
    ...
    // 判断当前文件是否在配置的目录里
    if(fileInfo.dir.indexOf(tplPath) < 0) {
        return false;
    }
    // 判断version是否存在
    ...
    if(fileInfo.dir.indexOf(tplPath) > -1) {
        ...
        content = replaceUrl(content, options.regPre, options.prefix, version);
    }
